...
where $\mathcal{F}$ is in ampere-turns [S. K. Bhattacharya, *Basic Electricals and Electronics*]. The mmf scales linearly with both $N$ and $I$.

#### Magnetic flux and flux density

The magnetic effect set up in the core is described by the **magnetic flux** $\Phi$, measured in webers (Wb). Flux tells how much magnetic field passes through a given cross-section [BIPM, *The International System of Units (SI) Brochure*](https://www.bipm.org/en/publications/si-brochure/). If the flux is spread over a cross-sectional area $A$, the **magnetic flux density** is
...
in ampere-turns per metre, where $l$ is the mean length of the magnetic path [B. L. Theraja and A. K. Theraja, *A Textbook of Electrical Technology, Vol. 1*]. Flux density and field intensity are related by
...
in which $\mu_0$ is the permeability of free space and $\mu_r$ is the **relative permeability** of the material. Air has $\mu_r \approx 1$; iron and silicon steel have values orders of magnitude larger, though $\mu_r$ itself varies with the level of magnetization [V. K. Mehta and Rohit Mehta, *Principles of Electrical Engineering and Electronics*]. This is why an iron-cored coil produces so much more flux than the same coil in air.
...
and is always greater than 1. Leakage flux influences coupling, voltage regulation, stray fields, and unwanted heating of nearby metal parts [Murata, *Technical Terms for Inductors*](https://www.murata.com/en-us/products/inductor/overview/learn/glossary).
...
Soft materials used in transformer cores show narrow loops; hard materials used in permanent magnets show wide ones [V. K. Mehta and Rohit Mehta, *Principles of Electrical Engineering and Electronics*].

#### Core losses

The area enclosed by the hysteresis loop equals the energy dissipated per cycle per unit volume of core material [B. L. Theraja and A. K. Theraja, *A Textbook of Electrical Technology, Vol. 1*]. This energy appears as heat, and because the loss is per cycle, the **hysteresis loss** rises with frequency. Choice of a soft magnetic material such as silicon steel keeps the loop narrow and the loss small at power frequencies.

A changing flux also induces circulating currents inside the conducting core itself. These **eddy currents** dissipate $I^2R$ heat in the core [OpenStax, *University Physics Volume 2, 13.5 Eddy Currents*](https://openstax.org/books/university-physics-volume-2/pages/13-5-eddy-currents). Eddy-current loss is controlled by:
...
where $e$ is in volts [OpenStax, *University Physics Volume 2, 13.1 Faraday's Law*](https://openstax.org/books/university-physics-volume-2/pages/13-1-faradays-law). For many elementary problems the average induced emf is sufficient:
...
where $L$ is the self-inductance in henries (H) [Murata, *Technical Terms for Inductors*](https://www.murata.com/en-us/products/inductor/overview/learn/glossary). One henry corresponds to an induced emf of 1 V for a rate of current change of $1\ \text{A/s}$ [BIPM, *The International System of Units (SI) Brochure*](https://www.bipm.org/en/publications/si-brochure/). By Lenz's law the sign is opposing: rising current induces an emf that retards it, and falling current induces an emf that tries to sustain it. This is why an inductor current cannot change instantaneously, and why breaking the current in a relay coil or motor winding produces a high-voltage spike across the contacts — an effect commonly suppressed by a freewheeling diode.
...
where $M$ is the mutual inductance between the two coils. This is the principle of the transformer: changing current in the primary sets up a changing flux in the common core, which links the secondary and induces an emf there [Britannica, *electromagnetic induction*](https://www.britannica.com/science/electromagnetic-induction). No charge crosses from one winding to the other; energy transfers through the magnetic field alone, which is why transformer windings remain electrically isolated.
...
**Fleming's Left-Hand Rule** gives the direction of force on a current-carrying conductor in a magnetic field [Edward Hughes, *Hughes Electrical and Electronic Technology*]. Hold the thumb, forefinger, and middle finger of the left hand mutually perpendicular; then
...
### 2.4 Worked Example: Reading an Inductor Datasheet

Real components expose the ideas of this chapter in concise numerical form. Consider the specification table in the [TDK PLEC69B thin-film power inductor datasheet](https://product.tdk.com/system/files/dam/doc/product/inductor/inductor/smd/catalog/inductor_commercial_power_plec69_en.pdf), and in particular the part `PLEC69BCA100M-1PT00` [TDK, *Inductors for power circuits, PLEC69B type datasheet*]:
...
Magnetic circuits appear inside every transformer, relay, contactor, meter, stabilizer, and motor winding; induction underlies every generator, transformer, and switched-mode converter. In DC-DC converters, inverters, and filters for battery-charging and solar PV systems, core saturation, inductance value, and leakage flux set efficiency, ripple, and thermal performance. In instrumentation and control, relays and inductive sensors generate the switching spikes that drive the design of snubbers and freewheeling diodes. Even in digital systems, magnetic components remain in power supplies, isolation transformers, and solenoid drivers.

## Chapter Summary

- **Magnetomotive force:** $\mathcal{F} = NI$ (ampere-turns).
- **Flux and flux density:** $\Phi$ in webers; $B = \Phi/A$ in tesla.
- **Field intensity and permeability:** $H = NI/l$; $B = \mu H$, with $\mu = \mu_0\mu_r$.
- **Reluctance:** $\mathcal{R} = l/(\mu A)$; $\Phi = \mathcal{F}/\mathcal{R}$; reluctances add in series.
- Air gaps dominate total reluctance because $\mu_r \approx 1$ in air.
- **Leakage factor:** $\lambda = \Phi_t/\Phi_u > 1$.
- The **B-H curve** shows saturation; soft, hard, and ferrite materials are chosen for different roles.
- **Hysteresis loss** and **eddy-current loss** together form the core loss; lamination and material choice control both.
- **Faraday's law:** $e = -N\,d\Phi/dt$; **Lenz's law** sets its direction.
- **Dynamically induced emf:** $E = Blv$ for perpendicular motion.
- **Self- and mutual inductance:** $e = -L\,di/dt$; $e_2 = -M\,di_1/dt$; coupling $k = M/\sqrt{L_1 L_2}$.
- **Fleming's Left-Hand Rule** gives force direction (motor action); **Right-Hand Rule** gives induced-current direction (generator action).
...
1. [V. K. Mehta and Rohit Mehta, *Principles of Electrical Engineering and Electronics*](https://www.schandpublishing.com/books/technical/general-engineering/principles-electrical-engineering-electronics/9789352534798/)
   A standard introductory text that explains magnetic circuits, induction, and machine basics in a diploma-friendly style.

2. [Murata, *Technical Terms for Inductors*](https://www.murata.com/en-us/products/inductor/overview/learn/glossary)
   A practical manufacturer reference for inductance, leakage flux, saturation current, and magnetic coupling terms used in real components.

3. [OpenStax, *University Physics Volume 2, 13.1 Faraday's Law*](https://openstax.org/books/university-physics-volume-2/pages/13-1-faradays-law)
   A clear and reliable treatment of flux, induced emf, and the mathematical form of Faraday's law.

4. [Britannica, *Lenz's law*](https://www.britannica.com/science/Lenzs-law)
   A concise explanation of the direction of induced current and its link to conservation of energy.

5. [TDK, *Inductors for power circuits, PLEC69B type datasheet*](https://product.tdk.com/system/files/dam/doc/product/inductor/inductor/smd/catalog/inductor_commercial_power_plec69_en.pdf)
   A manufacturer datasheet showing how inductance, saturation current, DC resistance, and temperature limits are specified in practice.
