...
Thermal management deals with the heat produced by semiconductor loss. Every watt dissipated in a MOSFET, IGBT, or diode appears at the device junction, and converter reliability depends on carrying that heat through the package and heat sink to the surrounding air. A converter may satisfy its electrical specifications and still age prematurely if junction temperature remains too high during normal operation.

This chapter covers the main device-loss mechanisms, the thermal-resistance model used for preliminary heat-sink selection, and the basic distinction between natural and forced cooling [ST AN4783], [Infineon AN-1057].

## Prerequisites check

- Recall from Chapters 1.4 and 1.5 that MOSFETs and IGBTs do not switch instantaneously, so voltage and current overlap during switching.
- Recall from Chapter 2.1 that gate-drive strength changes switching speed and therefore affects switching loss.
- Recall from Chapter 2.2 that snubbers can reduce overshoot and ringing, but may also dissipate power.
- Be comfortable with the power relations $P = VI$ and, for a resistor, $P = I^2R$.
- Distinguish ambient temperature from device temperature, even if the thermal-resistance model is new.

If switching waveforms are unclear, review Chapters 1.4, 1.5, and 2.1 before proceeding.
...
Different devices express these losses differently. A MOSFET usually represents conduction loss through ON-state resistance. An IGBT or diode is often approximated by an ON-state voltage drop. At high switching frequency, switching loss may dominate; at lower frequency and high current, conduction loss may dominate [Mohan, Undeland, Robbins, *Power Electronics*].
...
Conduction loss scales with the square of current. If current doubles, conduction loss becomes approximately four times larger.
...
The doubled current produces four times the loss.
...
In preliminary calculations, MOSFET conduction loss is usually treated as resistive, while IGBT and diode conduction loss is often treated as voltage-drop times current. More detailed models are possible, but these forms are adequate for thermal estimates [Mohan, Undeland, Robbins, *Power Electronics*], [Rashid, *Power Electronics: Circuits, Devices and Applications, 4e*].

#### Why conduction loss usually rises with temperature

Temperature affects the loss as well as the temperature rise. For many power MOSFETs, $R_{DS(\text{on})}$ increases with junction temperature. For many IGBTs, $V_{CE(\text{sat})}$ also changes with temperature. The STGWA40HP65FB datasheet, for example, shows $V_{CE(\text{sat})}$ increasing with junction temperature in its static characteristics and Figure 5 [STGWA40HP65FB Datasheet].

The consequence is a reinforcing loop:
...
Higher voltage, higher current, slower switching, or higher switching frequency all increase switching loss.
...
Datasheets often provide switching energy directly, which leads to a more practical design relation:
...
A stronger gate driver can reduce switching time and therefore switching loss, but excessive drive strength may increase $dv/dt$, $di/dt$, and EMI. A snubber can reduce overshoot and stress, but it may also dissipate additional power. Datasheet current ratings likewise have meaning only under their stated thermal conditions.
...
This relation underlies preliminary heat-sink selection.
...
Preliminary heat-sink selection follows a short procedure:
...
This is the largest sink-to-ambient thermal resistance acceptable for the design.
...
The heat sink is only one part of the thermal path. Real device packages and heat sinks are not perfectly flat, and trapped air is a poor thermal conductor. A **thermal interface material**, or TIM, fills surface gaps and lowers thermal resistance. ST notes that grease or insulating layers such as mica, ceramic, or silicone can be used, and that contact pressure matters because the case-to-heat-sink resistance arises from real, nonideal contact surfaces [ST AN4783].
...
Steady-state thermal resistance is appropriate for continuous average heating. Fault pulses, startup surges, current bursts, and intermittent overloads may instead be short compared with the thermal time constant of the package and heat sink.
...
Use $R_{\text{th}}$ for steady or average heating, and use $Z_{\text{th}}(t)$ when pulse duration matters. Datasheet values such as $R_{\text{thJA}}$ and total power dissipation must always be read with their stated mounting and temperature conditions. Heat-sink performance is not fixed either; orientation, airflow, surface area, and enclosure conditions all change the effective sink-to-ambient thermal resistance [ST AN4783], [Infineon AN-1057].

### 2.4.3 Forced cooling - overview

#### Natural and forced convection

Natural cooling is simple, silent, and free of moving parts. If semiconductor losses are modest and sufficient heat-sink area is available, natural convection may be adequate.
...
Thermal resistance decreases when either the surface area increases or the heat-transfer coefficient increases. A fan mainly improves the second factor by moving more air past the fins.

With stronger airflow, the same heat sink can carry more power, or a smaller heat sink can carry the same power. Forced-air cooling is therefore common in compact UPS systems, EV chargers, telecom rectifiers, and medium-power PV inverter cabinets.
...
A good heat sink can perform poorly inside a poor airflow path. Forced cooling also introduces reliability questions that natural convection avoids: fan failure, dust accumulation, acoustic noise, blocked passages, and the possible need for airflow monitoring or thermal-shutdown backup.

#### Comparing the main cooling approaches

At higher power density, the cooling method becomes part of the converter architecture rather than a packaging detail. Liquid cooling is not treated in detail at this level, but it belongs beside natural and forced-air cooling in the overall design picture.
...
Local ambient temperature must be interpreted correctly. In converter practice, the air around the heat sink may be much hotter than the room or outdoor temperature because the enclosure already contains other heat-generating components.
...
The following exercise uses the [STGWA40HP65FB datasheet](https://www.st.com/resource/en/datasheet/stgwa40hp65fb.pdf), which brings thermal limit, thermal resistance, conduction behavior, switching energy, and transient thermal information together in one device [STGWA40HP65FB Datasheet].
...
Those two numbers show that the device rating is thermal as well as electrical. The junction can rise only a limited amount above the case, and the allowable power dissipation depends on that temperature rise.
...
This interprets the junction-to-case limit, not a recommended operating target. It shows how strongly allowable dissipation depends on case temperature.
...
Figure 1 in the datasheet, "Power dissipation vs case temperature," makes the condition explicit [STGWA40HP65FB Datasheet]. As case temperature rises, the allowable dissipation falls.
...
Electrical loss and thermal behavior are coupled. If junction temperature rises, switching energy may rise as well, which increases dissipation and further raises device temperature.
...
The same questions should be asked of any power-device datasheet. What junction-temperature limit is allowed? What thermal resistance separates the junction from the case? Under what case temperature is the quoted power-dissipation rating valid? How strongly do switching energy and conduction characteristics change with temperature? Is pulsed operation governed by steady-state resistance or by transient thermal impedance? Those questions convert a datasheet from a list of ratings into a thermal design tool.

## Chapter summary

Thermal management begins with loss estimation. Conduction loss depends on ON-state resistance or ON-state voltage drop, switching loss depends on voltage-current overlap and switching energy, and both often increase as junction temperature rises. Electrical and thermal behavior must therefore be treated together.

Thermal design then converts loss into temperature rise through the path from junction to ambient. Equations (9.5) to (9.8) provide a preliminary method for steady-state junction-temperature estimation and heat-sink selection, while transient thermal impedance extends the analysis to pulsed operation. In renewable-energy converters, chargers, UPS systems, and similar equipment, long operating hours, enclosure heating, and power density make these thermal limits central to reliability as well as efficiency. Natural convection, forced-air cooling, and liquid cooling differ mainly in how effectively they reduce the effective device-to-environment thermal resistance.
