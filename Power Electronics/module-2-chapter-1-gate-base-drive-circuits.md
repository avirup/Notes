...
The drive circuit forms the interface between a low-power controller and a power semiconductor. It must apply the correct control voltage or current, deliver adequate source and sink capability, establish the correct electrical reference, and, in many converters, provide level shifting or galvanic isolation. A power device with good voltage and current ratings can still switch poorly or fail if the drive circuit is weak, noisy, or incorrectly referenced.

For a power BJT this interface is a **base-drive circuit**; for a MOSFET or IGBT it is a **gate-drive circuit**. This chapter examines the requirements of a good driver, the role of gate charge and peak current, the totem-pole output stage, dedicated driver ICs, high-side and low-side driving, and isolated-drive methods. These topics determine switching quality in practical converters such as PV inverters, battery chargers, UPS hardware, motor drives, and EV auxiliary supplies.
...
The discussion assumes familiarity with basic switching waveforms, device terminals, and DC-bus voltage levels.

### 2.1.1 Requirements of a good drive circuit

A drive circuit must force the device into the intended ON or OFF state at the intended time and with the intended electrical reference. The control requirement depends on the device type. A power BJT requires control current while it is ON. A power MOSFET or IGBT requires charge transfer during switching; under steady DC gate bias, the ideal input current is very small [Infineon AN_2203 Gate Drive].

For a power BJT, the base-drive requirement can be estimated from the forced-beta relation
...
That requirement is far beyond the capability of an ordinary logic output, so the base-drive stage becomes a power circuit in its own right.

For MOSFETs and IGBTs, the control terminal is insulated, but rapid charging and discharging are still essential. Slow gate charging extends the time spent in the transition region, increases switching loss, and raises the risk of cross-conduction in bridge circuits.

Table 2.1 lists the main requirements of a good drive circuit.
...
Two requirements deserve particular emphasis. First, the driver output must be referenced to the local source or emitter terminal, not to earth or controller ground. A MOSFET responds to $V_{GS}$ and an IGBT responds to $V_{GE}$; if the source or emitter moves, the drive reference must move with it or must be isolated from it. Second, the driver must present low output impedance so that the control terminal remains firmly at the intended voltage in the presence of noise, parasitic inductance, and Miller-coupled disturbances.
...
Gate charge is often more useful than a single capacitance value for first estimates because the effective capacitances vary with operating condition [Infineon AN_2203 Gate Drive]. If the driver current is treated as approximately constant over the critical part of the transition, then
...
The corresponding estimate of average gate-drive power is
...
These relations distinguish average drive power from peak gate current. A driver may dissipate little average power while still requiring several hundred milliamperes or several amperes of peak current to obtain fast switching.
...
The driver should therefore be able to source and sink on the order of $0.5 \text{ A}$, with additional margin for parasitics and operating variation.
...
The average gate-drive power is only about $36 \text{ mW}$, yet the peak current requirement is around $0.5 \text{ A}$. A microcontroller pin is therefore usually inadequate as a direct power-switch driver.

#### The Miller plateau

During turn-ON, gate voltage does not always rise at a uniform rate. There is often an interval in which the gate-voltage rise slows or nearly flattens while the drain-source or collector-emitter voltage is changing. This interval is the **Miller plateau**. During it, much of the gate current is diverted into the effective gate-drain or gate-collector feedback capacitance rather than into further gate-voltage rise [Infineon AN_2203 Gate Drive].

The Miller plateau corresponds to the collapse of device voltage and a large share of switching loss, so driver current remains important even when the gate voltage appears nearly stationary.
...
The standard output structure for a gate driver is the **totem-pole driver**, also called a **push-pull output stage**. One device in the driver sources current into the gate during turn-ON, and another device sinks current during turn-OFF. The result is a low-impedance path in both directions.

This arrangement is much stronger than passive charging or discharging through large resistances. A resistor can eventually move the required charge, but a totem-pole stage does so actively and quickly. Discrete driver stages built from small-signal BJTs or MOSFETs are often used to raise a logic signal to the required gate-drive level and to provide the necessary source and sink current [Infineon AN_2203 Gate Drive].

If a low-side MOSFET source is tied near ground, a 3.3 V controller pulse may still be insufficient for efficient switching. The gate may require 10 V to 12 V drive, and the transition may require substantial current. A level-shift stage followed by a totem-pole output stage can translate the logic pulse to the required amplitude and deliver strong gate current. In many designs the turn-OFF path is made stronger than the turn-ON path so that the device is pulled down firmly and false turn-ON from $C\,dv/dt$ coupling is less likely in half-bridge and full-bridge circuits [Infineon AN_2203 Gate Drive].
...
The same points should be checked in any driver datasheet: whether the device is a true gate driver or only an isolator, how strong the source and sink stages are, whether UVLO is included, what isolation and CMTI ratings are specified, and whether the part is intended for low-side, high-side, bootstrap-based, or fully isolated use.
...
In a half-bridge, when the low-side switch turns ON, the midpoint falls near the negative rail; when the high-side switch turns ON, the midpoint rises near the positive rail. The high-side driver must therefore operate on a floating reference.
...
This relation gives only a first estimate. In practice, $Q_{\text{TOT}}$ includes not only power-device gate charge but also driver quiescent current, leakage, and design margin.
...
Bootstrap driving is compact and economical, but the capacitor must be refreshed. The switching pattern must therefore return the node periodically to a condition that allows recharging. Bootstrap drive is well suited to many PWM bridge circuits, but it is not suitable for every duty cycle or operating mode.
...
**Galvanic isolation** means that no direct conductive path for steady current exists between the two sides of the interface. In gate driving, isolation may be required for safety, noise immunity, or level shifting across a large potential difference. The controller may sit on a low-voltage domain while the power switch moves through hundreds of volts under high common-mode transient stress.

The main isolated-drive families are opto-coupler-based drive, pulse-transformer-based drive, and integrated isolated gate-driver ICs.
...
Integration simplifies implementation but does not remove the underlying design questions. The designer must still consider isolation rating, channel arrangement, source and sink current, common-mode transient tolerance, and suitability for low-side, high-side, or half-bridge use.
...
- A drive circuit connects the low-power control stage to the power semiconductor and must provide the correct amplitude, current capability, reference, timing, and protection behavior.
- A BJT base-drive circuit must supply control current while the device is ON; a first switching estimate is $I_B \ge I_C/\beta_{\text{forced}}$.
- A MOSFET or IGBT gate draws little steady-state current, but switching requires charge transfer; the central relation is $Q_G = \int i_G(t)\,dt$ [Infineon AN_2203 Gate Drive].
- First estimates for switching time and average drive power are $t_{\text{sw,est}} \approx Q_G / I_G$ and $P_{\text{DRV,avg}} \approx Q_G V_{\text{DRV}} f_s$ [Infineon AN_2203 Gate Drive].
- The Miller plateau is the interval during which gate current continues to flow while device voltage is changing.
- A totem-pole driver provides low-impedance source and sink paths for rapid turn-ON and turn-OFF, and dedicated driver ICs commonly add UVLO and other protection features.
- Low-side drivers can often share a ground reference, whereas high-side drivers must operate on a floating reference.
- Bootstrap high-side drive is compact and widely used, but it depends on periodic refresh of the bootstrap capacitor.
- Isolated gate drive may be implemented with opto-couplers, pulse transformers, or integrated isolated driver ICs, each with distinct advantages and limitations.
